...
class MonthlyStateInterpreter:
    ...
    def __init__(
        ...
        #  Cost/return columns (ret, DolVol, BidAskSpread, VolMkt) stay
        #  RAW because the env uses their actual values for portfolio math.
        #  Only the feature columns fed into the neural network are scaled.
        # ================================================================
    ...
    def get_month_slot_arrays(self, yyyymm: int) -> Dict[str, np.ndarray]:
        """
        Return slot-aligned arrays via direct numpy indexing into the
        pre-computed cube.  Zero allocation, zero loops.
        """
        mi = self._month_to_idx.get(int(yyyymm))
        max_N = self.max_universe_size
        ...
        cube_slice = self._data_cube[mi, :max_N, :]
        return {
            "ret":          cube_slice[:, self._ret_idx],
            "DolVol":       cube_slice[:, self._dolvol_idx] if self._dolvol_idx >= 0 else np.zeros(max_N, dtype=np.float32),
            "BidAskSpread": cube_slice[:, self._spread_idx] if self._spread_idx >= 0 else np.zeros(max_N, dtype=np.float32),
            "VolMkt":       cube_slice[:, self._volmkt_idx] if self._volmkt_idx >= 0 else np.zeros(max_N, dtype=np.float32),
