...
def main(args):
    ...
    # For each model, get the history of training and validation.
    ...
    for target, scores in predictions.items():
        ...
        rows.append(row)
    # Ensemble metrics  
    if len(targets) > 1 and config.get("ensemble", {}).get("enabled", True):
        ...
        for g in group_test:
            sl = slice(cursor, cursor + g)
            for k in eval_at:
                ndcg_scores[k].append(ndcg_at_k(ensemble_scores[sl], ref_encoded[sl].astype(float), k))
                hit_scores[k].append(precision_at_k(ensemble_scores[sl], ref_encoded[sl].astype(float), k))
            cursor += g

        auc_ens = roc_auc_score((ref_encoded > np.median(ref_encoded)).astype(int), ensemble_scores)

        row = {
            "target": f"ensemble/{combination}",
            "AUC": auc_ens,
        }
        ...
        # Save X_test, y_test, group_test, and ensemble_scores in one dataframe 
        df_test = X_test.copy()
        for target in targets:
            df_test[target] = y_test[target]
